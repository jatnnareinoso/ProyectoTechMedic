...
router.get("/list/solicitadas/:id_usuario", async (req, res) => {
    ...
    try {
        const query_accesos_solicitados = `
            SELECT au.id_acc_usuario, au.id_accesos, acc.id_especialidad, MAX(con.fecha_hora) AS fecha_ultima_consulta
            ...
            GROUP BY au.id_acc_usuario, au.id_accesos, acc.id_especialidad
        `;
        ...
        const result = await client.query(query_accesos_solicitados);

        accesos_usuario.push(...result.rows)
